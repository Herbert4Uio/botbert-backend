...
12. CATERING Y EVENTOS: Si el cliente está solicitando una cotización o servicio de Catering/Eventos, recopila todos los detalles del evento (fecha, hora, lugar, personas, restricciones). Una vez recolectados, DEBES registrar la solicitud en el sistema usando 'generar_orden'. Como es una cotización y no una compra directa, usa valores por defecto: paymentType="EFECTIVO", paymentTiming="PAY_LATER", deliveryType="ENVIO", branchId="N/A", y en 'items' envía un array vacío []. Para el nombre, DEBES OBLIGATORIAMENTE usar customerFullName="[Nombre del Cliente]". OBLIGATORIO: Debes llenar el objeto 'eventDetails' en la llamada a la herramienta con toda la información recopilada. NO uses la herramienta hasta tener los datos del evento.
...
${modificationRules}`;
  ...
  return `
    ...
    IMPORTANTE: DEBES preguntarle explícitamente al cliente su Nombre Completo y su NIT para la factura ANTES de intentar usar la herramienta 'generar_orden'. No inventes nombres (ej. no uses "Cliente") ni asumas que no proporcionó NIT sin antes preguntar. Cuando generes la orden, INCLUYE ESTOS DATOS SIEMPRE en los campos 'customerFullName' y 'customerNit'.
