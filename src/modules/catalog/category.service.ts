...
export class CategoryService {
  ...
  async create(tenantId: string, data: any) {
    const created = new this.categoryModel({ ...data, tenantId: new Types.ObjectId(tenantId) });
    return created.save();
  }

  async findAll(tenantId: string) {
    return this.categoryModel.find({ tenantId: new Types.ObjectId(tenantId) }).exec();
  }

  async findOne(tenantId: string, id: string) {
    const category = await this.categoryModel.findOne({ _id: new Types.ObjectId(id), tenantId: new Types.ObjectId(tenantId) }).exec();
